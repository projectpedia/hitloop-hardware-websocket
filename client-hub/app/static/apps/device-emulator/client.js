...
const DEVICE_COMMANDS = new Set(['led', 'pattern', 'brightness', 'spring_param', 'vibrate', 'reset', 'status', 'ota']);
...
function getHeartbeatBrightness(now) {
    const PERIOD   = 2000;
    const FI1 = 60, FO1 = 150, PZ = 100, FI2 = 60, FO2 = 400; // pulse = 770ms, idle = 1230ms
    const t = now % PERIOD;
...
function computeLedColor(now) {
    switch (ledPattern) {
        ...
        case 'cycle': {
            ...
            return [rgb[0] * ledBrightness, rgb[1] * ledBrightness, rgb[2] * ledBrightness];
        }
...
window.draw = function() {
    ...
    fill(255);
    textAlign(CENTER);
    const wsStatus = ws ? (ws.readyState === WebSocket.OPEN ? 'connected' : (ws.readyState === WebSocket.CONNECTING ? 'connecting' : 'disconnected')) : 'disconnected';
    ...
    const isVibrating = now < vibEndTime;
    const vibStatus   = isVibrating ? `vibrating (${Math.ceil((vibEndTime - now) / 1000 * 10) / 10}s)` : '';
    const ledHex      = ledColor.map(v => Math.round(v).toString(16).padStart(2, '0')).join('');
    const ledStatus   = `led: #${ledHex}  pattern: ${ledPattern}  brightness: ${Math.round(ledBrightness * 255)}`;

    text(`status: ${wsStatus}  id: ${deviceId}  mode: ${controlMode}`, width/2, height - 54);
